...
Node<T>* LinkedList<T>::search(Node<T>* location, T data)
{
    ...
    while (curr != nullptr) {
        ...
        if (curr->data == data) {  // checks to see if data is in list
          ...
          return curr->prev; // return pointer to location for new data
        }
...
{
    Node<T> *curr = location;
    Node<T> *right;       // node that will be at the right of the new node
    right = curr->next;

    Node<T>* newNode = new Node<T>(data);  // create new node initialized with data
    curr->next = newNode;
    newNode->prev = curr;
    newNode->next = right;
    right->prev = newNode;
...
void LinkedList<T>::printData()
{
  ...
  curr->printData();    // print last node since the while loops ends before last node can be printed
}
...
void LinkedList<T>::print()
{
  ...
  curr->print();  // print last node since the while loops ends before last node can be printed
}
